...
files <- files %>% 
  str_subset('2-04救急車による搬送の有無の医療機関別MDC別集計') %>% 
  print()
...
# 前処理用の関数
cleanup_qq_mdc02 <- function(filepath){
  ...
  # 出力用に整形
  df <- df %>% 
    select(fy,no,mdc2cd,qq=value)
  
  df <- df %>% 
    mutate(qq=as.numeric(qq)) %>%
    print()
...
# テスト
filepath <- files[1]
test <- cleanup_qq_mdc02(filepath) %>%
  print()
...
# dbに書き込み
dbWriteTable(con, 'agg_qq_by_mdc2', df, overwrite = T,row.names = F)
